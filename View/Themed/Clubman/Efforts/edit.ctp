<!-- app/View/Efforts/edit.ctp -->
<h2>Wijzig "<?=$this->request['data']['Effort']['taskname']?>" prestatie voor <?=$member['Member']['name']?></h2>
<script>
		...
		<div class="efforts form">
			...
			<?php if ($loggedIn and ($this->Permission->iAmOneOf($cmCurrentRoles, ['root', 'admin']))) : ?>
				<div class="form-group">
					...
					<div class="col-sm-6">
						<?=$this->Form->input('member_id', array('label' => false, 'div' => false, 'class' => 'form-control', 'type' => 'select'))?>
					</div>
				</div>
			<?php else : ?>
